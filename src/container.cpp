	...
	if (item->isStackable()) {
		...
		if (index == INDEX_WHEREEVER) {
			...
			for (Item* containerItem : itemlist) {
				if (containerItem != item && containerItem->equals(item) && containerItem->getItemCount() < 250) {
					if (queryAdd(slotIndex++, *item, count, flags) == RETURNVALUE_NOERROR) {
						n += 250 - containerItem->getItemCount();
					}
				}
			}
		} else {
			const Item* destItem = getItemByIndex(index);
			if (item->equals(destItem) && destItem->getItemCount() < 250) {
				if (queryAdd(index, *item, count, flags) == RETURNVALUE_NOERROR) {
					n = 250 - destItem->getItemCount();
				}
			}
		}

		maxQueryCount = freeSlots * 250 + n;
		...
		uint32_t& flags)
{
	...
	if (autoStack && item->isStackable() && item->getParent() != this) {
		if (*destItem && (*destItem)->equals(item) && (*destItem)->getItemCount() < 250) {
			return this;
		}
		...
		for (Item* listItem : itemlist) {
			if (listItem != item && listItem->equals(item) && listItem->getItemCount() < 250) {
				*destItem = listItem;
				index = n;
				return this;
			}
