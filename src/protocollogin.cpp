...
void ProtocolLogin::getCharacterList(const std::string& accountNameOrEmail, const std::string& password, const std::string& token, uint16_t version)
{
    Account account;
    if (!IOLoginData::loginserverAuthentication(accountNameOrEmail, password, account)) {
    ...
    //Add session key
    output->addByte(0x28);
    output->addString(accountNameOrEmail + "\n" + password + "\n" + token + "\n" + std::to_string(ticks));
...
void ProtocolLogin::onRecvFirstMessage(NetworkMessage& msg)
{
    ...
    std::string accountNameOrEmail = msg.getString();
    if (accountNameOrEmail.empty()) {
        disconnectClient("Invalid account name or email.", version);
        return;
    }
    ...
    g_dispatcher.addTask(createTask(std::bind(&ProtocolLogin::getCharacterList, thisPtr, accountNameOrEmail, password, authToken, version)));
}
