...
class IOLoginData
{
	public:
		...
		static bool loginserverAuthentication(const std::string& nameOrEmail, const std::string& password, Account& account);
		static uint32_t gameworldAuthentication(const std::string& accountNameOrEmail, const std::string& password, std::string& characterName, std::string& token, uint32_t tokenTime);
	...
	private:
		...
		static void loadItems(ItemMap& itemMap, DBResult_ptr result, const std::string& tableName);
