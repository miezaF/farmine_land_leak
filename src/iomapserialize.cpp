...
void IOMapSerialize::loadHouseItems(Map* map)
{
    ...
    DBResult_ptr result = Database::getInstance().storeQuery("SELECT `data`, `json_data` FROM `tile_store`");
    if (!result) {
        return;
    }
    ...
    do {
        ...
        while (item_count--) {
            if (!loadItem(propStream, tile)) {
                //std::cout << "ERROR: Failed to load item at (" << x << ", " << y << ", " << static_cast<int>(z) << ")" << std::endl;
            }
        }

        std::string jsonData = result->getString("json_data");
        if (!jsonData.empty()) {
            try {
                auto json = nlohmann::json::parse(jsonData);
                //std::cout << "INFO: Successfully parsed JSON data: " << json.dump() << std::endl;
            } catch (const std::exception& e) {
                std::cout << "ERROR: Failed to parse JSON data. Exception: " << e.what() << std::endl;
            }
        }
...
bool IOMapSerialize::saveHouseItems()
{
    ...
    DBInsert stmt("INSERT INTO `tile_store` (`house_id`, `data`, `json_data`) VALUES ");
    ...
    for (const auto& it : g_game.map.houses.getHouses()) {
        ...
        for (HouseTile* tile : house->getTiles()) {
            ...
            if (attributesSize > 0) {
                nlohmann::json jsonData = {};  // Supondo que você tenha uma forma de obter o JSON dos itens
                if (!stmt.addRow(fmt::format("{:d}, {:s}, {:s}", house->getId(), db.escapeBlob(attributes, attributesSize), db.escapeString(jsonData.dump())))) {
                    return false;
                }
                stream.clear();
            }
...
bool IOMapSerialize::loadItem(PropStream& propStream, Cylinder* parent)
{
    ...
    // Adiciona log para mostrar o item ID
    //std::cout << "DEBUG: Item ID (itemType): " << id << std::endl;
