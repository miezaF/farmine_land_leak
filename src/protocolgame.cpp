...
void ProtocolGame::AddCreature(NetworkMessage& msg, const Creature* creature, bool known, uint32_t remove)
{
    ...
    if (known) {
        ...
        const Player* playerCreature = creature->getPlayer();
        if (monster && monster->getLevel() > 0) {
            msg.addString(creature->getName() + " [" + std::to_string(monster->getLevel()) + "]");
        } else if (playerCreature || creature == player) {
            msg.addString(creature->getName() + " [Lv: " + std::to_string(playerCreature ? playerCreature->getLevel() : 0) + "]");
        } else {
            msg.addString(creature->getName());
        }
    }
