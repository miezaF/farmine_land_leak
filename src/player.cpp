...
int32_t Player::getDefense() const
{
	...
	return (armorValue / 4. + 2.23) * defenseValue * 0.15 * 2 * vocation->defenseMultiplier;
}
...
{
	switch (fightMode) {
		case FIGHTMODE_ATTACK: return (OTSYS_TIME() - lastAttack) < getAttackSpeed() ? 1.0f : 1.0f;
		case FIGHTMODE_BALANCED: return (OTSYS_TIME() - lastAttack) < getAttackSpeed() ? 1.0f : 1.0f;
...
void Player::addSkillAdvance(skills_t skill, uint64_t count)
{
	...
	while ((skills[skill].tries + count) >= nextReqTries) {
		...
		int adjustedSkillLevel = skills[skill].level - 10;
		sendTextMessage(MESSAGE_EVENT_ADVANCE, fmt::format("You advanced to {:s} skill {:d}.", getSkillName(skill), adjustedSkillLevel));
...
void Player::addExperience(Creature* source, uint64_t exp, bool sendText/* = false*/)
{
	...
	if (prevLevel != level) {
		...
		sendTextMessage(MESSAGE_EVENT_ADVANCE, fmt::format("You have reached Level {:d}.", prevLevel, level));
	}
