...
Item* Item::CreateItem(PropStream& propStream)
{
	...
	// Desserializar os atributos do item
...
Item::Item(const uint16_t type, uint16_t count /*= 0*/) :
	id(type),
	jsonData("{}") // Inicializa jsonData com "{}"
{
...
void Item::setJsonData(const nlohmann::json& data) {
    this->jsonData = data;
}

nlohmann::json Item::getJsonData() const {
    return jsonData;
}

void Item::addOrUpdateJsonData(const std::string& key, const nlohmann::json& value) {
    this->jsonData[key] = value;
}

void Item::updateJsonField(const std::string& key, const nlohmann::json& value) {
    if (this->jsonData.contains(key)) {
        this->jsonData[key] = value;
    }
}
...
Attr_ReadValue Item::readAttr(AttrTypes_t attr, PropStream& propStream)
{
	switch (attr) {
		case ATTR_COUNT: {
			...
			if (size == sizeof(uint8_t) || size == 0x4)
			{
				uint8_t count8;
				if (!propStream.read<uint8_t>(count8))
				{
				...
				uint16_t count16;
				if (!propStream.read<uint16_t>(count16))
				{
					return ATTR_READ_ERROR;
				}
			setSubType(count16);
		...
		case ATTR_ACTION_ID: {
			...
			if (!propStream.read<uint16_t>(actionId)) {
				std::cout << "ERROR: Failed to read ATTR_ACTION_ID" << std::endl;
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_UNIQUE_ID: {
			...
			if (!propStream.read<uint16_t>(uniqueId)) {
				std::cout << "ERROR: Failed to read ATTR_UNIQUE_ID" << std::endl;
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_TEXT: {
			...
			if (!propStream.readString(text)) {
				std::cout << "ERROR: Failed to read ATTR_TEXT" << std::endl;
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_WRITTENDATE: {
			...
			if (!propStream.read<uint32_t>(writtenDate)) {
				std::cout << "ERROR: Failed to read ATTR_WRITTENDATE" << std::endl;
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_WRITTENBY: {
			...
			if (!propStream.readString(writer)) {
				std::cout << "ERROR: Failed to read ATTR_WRITTENBY" << std::endl;
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_DESC: {
			std::string text;
			if (!propStream.readString(text)) {
				std::cout << "ERROR: Failed to read ATTR_DESC" << std::endl;
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_CHARGES: {
			...
			if (!propStream.read<uint16_t>(charges)) {
				std::cout << "ERROR: Failed to read ATTR_CHARGES" << std::endl;
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_DURATION: {
			...
			if (!propStream.read<int32_t>(duration)) {
				std::cout << "ERROR: Failed to read ATTR_DURATION" << std::endl;
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_DECAYING_STATE: {
			...
			if (!propStream.read<uint8_t>(state)) {
				std::cout << "ERROR: Failed to read ATTR_DECAYING_STATE" << std::endl;
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_NAME: {
			...
			if (!propStream.readString(name)) {
				std::cout << "ERROR: Failed to read ATTR_NAME" << std::endl;
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_ARTICLE: {
			...
			if (!propStream.readString(article)) {
				std::cout << "ERROR: Failed to read ATTR_ARTICLE" << std::endl;
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_PLURALNAME: {
			...
			if (!propStream.readString(pluralName)) {
				std::cout << "ERROR: Failed to read ATTR_PLURALNAME" << std::endl;
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_WEIGHT: {
			uint32_t weight = 0; // Definindo o valor de weight como 0
			// Removendo a leitura do propStream
		...
		case ATTR_ATTACK: {
			...
			if (!propStream.read<int32_t>(attack)) {
				std::cout << "ERROR: Failed to read ATTR_ATTACK" << std::endl;
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_ATTACK_SPEED: {
			...
			if (!propStream.read<uint32_t>(attackSpeed)) {
				std::cout << "ERROR: Failed to read ATTR_ATTACK_SPEED" << std::endl;
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_DEFENSE: {
			...
			if (!propStream.read<int32_t>(defense)) {
				std::cout << "ERROR: Failed to read ATTR_DEFENSE" << std::endl;
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_EXTRADEFENSE: {
			...
			if (!propStream.read<int32_t>(extraDefense)) {
				std::cout << "ERROR: Failed to read ATTR_EXTRADEFENSE" << std::endl;
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_ARMOR: {
			...
			if (!propStream.read<int32_t>(armor)) {
				std::cout << "ERROR: Failed to read ATTR_ARMOR" << std::endl;
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_HITCHANCE: {
			...
			if (!propStream.read<int8_t>(hitChance)) {
				std::cout << "ERROR: Failed to read ATTR_HITCHANCE" << std::endl;
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_SHOOTRANGE: {
			...
			if (!propStream.read<uint8_t>(shootRange)) {
				std::cout << "ERROR: Failed to read ATTR_SHOOTRANGE" << std::endl;
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_DECAYTO: {
			...
			if (!propStream.read<int32_t>(decayTo)) {
				std::cout << "ERROR: Failed to read ATTR_DECAYTO" << std::endl;
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_WRAPID: {
			...
			if (!propStream.read<uint16_t>(wrapId)) {
				std::cout << "ERROR: Failed to read ATTR_WRAPID" << std::endl;
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_STOREITEM: {
			...
			if (!propStream.read<uint8_t>(storeItem)) {
				std::cout << "ERROR: Failed to read ATTR_STOREITEM" << std::endl;
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_TIER: {
			if (!propStream.skip(1)) {
				std::cout << "ERROR: Failed to skip 1 byte for ATTR_OPENCONTAINER or ATTR_TIER" << std::endl;
				return ATTR_READ_ERROR;
			}
			break;
		}
		...
		case ATTR_PODIUMOUTFIT: {
			if (!propStream.skip(15)) {
				std::cout << "ERROR: Failed to skip 15 bytes for ATTR_PODIUMOUTFIT" << std::endl;
				return ATTR_READ_ERROR;
			}
			break;
		}
		...
		case ATTR_DEPOT_ID: {
			if (!propStream.skip(2)) {
				std::cout << "ERROR: Failed to skip 2 bytes for ATTR_DEPOT_ID" << std::endl;
				return ATTR_READ_ERROR;
			}
			break;
		}
		...
		case ATTR_HOUSEDOORID: {
			if (!propStream.skip(1)) {
				std::cout << "ERROR: Failed to skip 1 byte for ATTR_HOUSEDOORID" << std::endl;
				return ATTR_READ_ERROR;
			}
			break;
		}
		...
		case ATTR_SLEEPERGUID: {
			if (!propStream.skip(4)) {
				std::cout << "ERROR: Failed to skip 4 bytes for ATTR_SLEEPERGUID" << std::endl;
				return ATTR_READ_ERROR;
			}
			break;
		}
		...
		case ATTR_SLEEPSTART: {
			if (!propStream.skip(4)) {
				std::cout << "ERROR: Failed to skip 4 bytes for ATTR_SLEEPSTART" << std::endl;
				return ATTR_READ_ERROR;
			}
			break;
		}
		...
		case ATTR_TELE_DEST: {
			if (!propStream.skip(5)) {
				std::cout << "ERROR: Failed to skip 5 bytes for ATTR_TELE_DEST" << std::endl;
				return ATTR_READ_ERROR;
			}
			break;
		}
		...
		case ATTR_CONTAINER_ITEMS: {
			std::cout << "ERROR: ATTR_CONTAINER_ITEMS is not supported" << std::endl;
			return ATTR_READ_ERROR;
		}
		...
		case ATTR_CUSTOM_ATTRIBUTES: {
			...
			if (!propStream.read<uint64_t>(size)) {
				std::cout << "ERROR: Failed to read size for ATTR_CUSTOM_ATTRIBUTES" << std::endl;
				return ATTR_READ_ERROR;
			}
			...
			for (uint64_t i = 0; i < size; i++) {
				...
				if (!propStream.readString(key)) {
					std::cout << "ERROR: Failed to read key for custom attribute" << std::endl;
					return ATTR_READ_ERROR;
				};
				...
				if (!val.unserialize(propStream)) {
					std::cout << "ERROR: Failed to unserialize custom attribute value for key: " << key << std::endl;
					return ATTR_READ_ERROR;
				}
		...
		default:
			std::cout << "ERROR: Unknown attribute type " << static_cast<int>(attr) << std::endl;
			return ATTR_READ_ERROR;
	}
...
{
    uint8_t attr_type;
    static std::unordered_set<uint16_t> ignoredItemTypes = {1539, 1540, 1541};
    static int ignoredItemCount = 0;

    while (propStream.read<uint8_t>(attr_type) && attr_type != 0) {
        if (ignoredItemTypes.find(attr_type) != ignoredItemTypes.end()) {
            ignoredItemCount++;
            continue;
        }
    ...
    size_t remainingSize = propStream.size();
    if (remainingSize > 0) {
        std::vector<char> remainingData(remainingSize);
        for (size_t i = 0; i < remainingSize; ++i) {
            if (!propStream.read<char>(remainingData[i])) {
                std::cout << "ERROR: Failed to read remaining data for jsonData. Data size: " << remainingSize << std::endl;
                jsonData = "{}"; // Define jsonData como "{}" em caso de falha
                return true;
            }
        }

        std::string remainingStr(remainingData.begin(), remainingData.end());
        //std::cout << "DEBUG: Remaining data for jsonData: " << remainingStr << std::endl;
        try {
            // Tenta parsear o JSON, se falhar define como "{}"
            auto parsedJson = nlohmann::json::parse(remainingStr);
            jsonData = parsedJson.dump();
            //std::cout << "INFO: Successfully read jsonData: " << jsonData << std::endl;
        } catch (const std::exception& e) {
            //std::cout << "ERROR: Invalid JSON data. Exception: " << e.what() << std::endl;
            //std::cout << "DEBUG: Invalid JSON data content: " << remainingStr << std::endl;
            jsonData = "{}";
        }
    } else {
        jsonData = "{}"; // Define jsonData como "{}" se não houver dados restantes
    }

    if (ignoredItemCount > 0) {
        std::cout << "INFO: Ignored item types " << ignoredItemCount << " times." << std::endl;
    }
...
void Item::serializeAttr(PropWriteStream& propWriteStream) const
{
    ...
    // Serializa atributos customizados, se existirem
    if (hasAttribute(ITEM_ATTRIBUTE_CUSTOM)) {
        ...
        for (const auto &entry : *customAttrMap) {
            // Serializa chave e valor
            propWriteStream.writeString(entry.first);
            entry.second.serialize(propWriteStream);
        }
    }

    // Salva jsonData se não estiver vazio e diferente de "{}"
    if (!jsonData.empty() && jsonData != "{}") {
        propWriteStream.writeString(jsonData);
    }

    // Indica o fim dos atributos
