...
// Implementação da função getJsonData como membro da classe DBResult
nlohmann::json DBResult::getJsonData(const std::string& s) const
{
	std::string jsonDataStr = getString(s);  // Obtém o campo como string
	if (jsonDataStr.empty()) {
		return nlohmann::json::object();  // Retorna um objeto JSON vazio se a string estiver vazia
	}

	try {
		return nlohmann::json::parse(jsonDataStr);  // Faz o parsing da string para um objeto JSON
	} catch (const nlohmann::json::parse_error& e) {
		std::cout << "[Warning] json_data inválido para coluna " << s << ". Definindo como {}." << std::endl;
		return nlohmann::json::object();  // Retorna um objeto JSON vazio em caso de erro de parsing
	}
}
