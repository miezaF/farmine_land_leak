...
bool Game::internalPlaceCreature(Creature* creature, const Position& pos, bool extendedPos /*= false*/, bool forced /*= false*/)
{
	...
	// Adicionando o parâmetro forced para permitir o spawn forçado
                                   ...
                                   Item* item, uint32_t count, Item** _moveItem, uint32_t flags /*= 0*/, Creature* actor/* = nullptr*/, Item* tradeItem/* = nullptr*/, const Position* fromPos /*= nullptr*/, const Position* toPos/*= nullptr*/)
{
	...
	if (item->isStackable()) {
		...
		if (item->equals(toItem)) {
			//
			n = std::min<uint32_t>(MAX_STACK_COUNT - toItem->getItemCount(), m);
                                  ...
                                  uint32_t flags, bool test, uint32_t& remainderCount)
{
	...
	*/
	...
	if (item->isStackable() && item->equals(toItem)) {
		...
		uint32_t n = std::min<uint32_t>(MAX_STACK_COUNT - toItem->getItemCount(), m);
...
void Game::addMoney(Cylinder* cylinder, uint64_t money, uint32_t flags /*= 0*/)
{
	...
	while (crystalCoins > 0) {
		const uint16_t count = std::min<uint32_t>(MAX_STACK_COUNT, crystalCoins);
	...
	uint16_t platinumCoins = money / MAX_STACK_COUNT;
	if (platinumCoins != 0) {
		...
		money -= platinumCoins * MAX_STACK_COUNT;
	}
...
void Game::playerEquipItem(uint32_t playerId, uint16_t spriteId)
{
	...
	if (slotItem && slotItem->getID() == it.id && (!it.stackable || slotItem->getItemCount() == MAX_STACK_COUNT || !equipItem)) {
                              ...
                              uint32_t tradePlayerId, uint16_t spriteId)
{
	...
	if (tradeContainer && tradeContainer->getItemHoldingCount() + 1 > MAX_STACK_COUNT) {
		player->sendCancelMessage("You can only trade up to" + std::to_string(MAX_STACK_COUNT) + "objects at once.");
		return;
	}
...
bool Game::playerSaySpell(Player* player, SpeakClasses type, const std::string& text)
{
    ...
    if (result == TALKACTION_BREAK) {
        return true; // Magia tratada por TalkAction, não prossegue.
    }

    result = g_spells->playerSaySpell(player, words);
    if (result == TALKACTION_BREAK) {
        // Aqui, uma magia foi lançada com sucesso. Retornamos true sem exibir a fala.
        return true;
    } else if (result == TALKACTION_FAILED) {
        // Se a ação de falar falhou, simplesmente retorne true.
        return true;
    }

    // Se nenhuma das condições acima for verdadeira, significa que nenhuma ação foi tomada.
    // Você pode optar por retornar false aqui se desejar tratar esse caso de alguma forma específica.
    return false;
}
